...
export default function Projects({ dict }: { dict: ProjectsDict }) {
  ...
  useEffect(() => {
    ...
    const container = viewport.parentElement; // The relative div containing buttons
    if (!container) return;
              ...
              <div className="flex gap-6">
                {projects?.map((project, index) => (
                  <motion.div
                    ...
                    className="min-w-64 sm:min-w-80 lg:min-w-96 xl:min-w-md 2xl:min-w-xl group relative bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow after:absolute after:inset-0 after:rounded-2xl after:border after:border-border after:pointer-events-none hover:after:border-primary/60"
                  >
